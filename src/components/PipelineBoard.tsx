    ...
    <div className="grid gap-4 xl:grid-cols-5">
      {PIPELINE_STAGES.map((stage) => {
        ...
        return (
          <div
            key={stage}
            className="kid-panel rounded-[24px] border border-ink/5 bg-white/78 p-4 shadow-soft"
          >
            <div className="mb-4 flex items-center justify-between gap-2">
              <p className="font-semibold text-ink">{stage}</p>
              <span
                className={`kid-ribbon rounded-full px-2.5 py-1 text-xs font-semibold ${stageClasses[stage]}`}
              >
                {stageLeads.length}
              </span>
            </div>
            ...
            <div className="space-y-3">
              {stageLeads.map((lead) => (
                <article
                  key={lead.id}
                  className="kid-panel rounded-[20px] border border-white/85 bg-white/96 p-4 shadow-soft transition duration-300 hover:-translate-y-0.5"
                >
                  ...
                  <p className="mt-3 text-sm text-ink/80">{lead.programInterest}</p>
